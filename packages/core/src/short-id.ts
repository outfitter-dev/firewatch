...
/**
 * Resolve a short ID to a full comment ID using provided entries.
 * This is a convenience function that builds the cache and resolves in one call.
 *
 * @param shortId - Short ID to resolve (with or without `@` prefix)
 * @param entries - Entries to search through
 * @returns Full comment ID or null if not found
 */
export function resolveShortIdFromEntries(
  shortId: string,
  entries: { id: string; repo: string; pr: number }[]
): string | null {
